...
public class KeycardOld_ZoneManager : CustomItem
{
    public override uint Id { get; set; } = 108;
    public override string Name { get; set; } = "区画管理者キーカード";
    ...
    public string SchemName = "OldKeycard_ZoneManager";
    ...
    private void AddSchem(PickupAddedEventArgs ev)
    {
        if (Check(ev.Pickup))
        {
            ...
            Timing.CallDelayed(0.01f, () =>
            {
                ...
                schematicObject.transform.localScale = new Vector3(1.4f, 5f, 1.4f);
                ...
                schematicObject.transform.localEulerAngles = new Vector3(180f, 60+180f,0f);
                //Timing.RunCoroutine(rot(schematicObject));
            });
        }
    }

    private IEnumerator<float> rot(SchematicObject schematicObject)
    {
        for (;;)
        {
            schematicObject.transform.localEulerAngles += new Vector3(0f, 10f, 0f);
            foreach (Player player in Player.List)
            {
                player.ShowHint(schematicObject.transform.localEulerAngles.ToString());
            }
