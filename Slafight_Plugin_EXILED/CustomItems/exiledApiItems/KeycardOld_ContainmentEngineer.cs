...
public class KeycardOld_ContainmentEngineer : CustomItem
{
    public override uint Id { get; set; } = 100;
    public override string Name { get; set; } = "収容技術者キーカード";
    ...
    private void AddSchem(PickupAddedEventArgs ev)
    {
        if (Check(ev.Pickup))
        {
            ...
            try
            {
                schematicObject = ObjectSpawner.SpawnSchematic("OldKeycard_ContainmentEngineer",Vector3.zero);
            }
            catch (Exception ex)
            {
