...
public static class ItemExtensions
{
    ...
    public static bool IsUpgradable(this Pickup item)
    {
        List<ItemType> allowed = new List<ItemType>()
        {
            ItemType.KeycardJanitor,
    ...
    public static void UpgradeItem(this Pickup item)
    {
        if (!item.IsUpgradable()) return;
        if (item.Type.IsKeycard())
        {
            if (item.Type == ItemType.KeycardJanitor)
            {
                ...
                {
                    item.Destroy();
                    Pickup.CreateAndSpawn(ItemType.KeycardScientist,item.Position);
                }
                else if (random == 1)
                {
                    item.Destroy();
                    Pickup.CreateAndSpawn(ItemType.KeycardZoneManager,item.Position);
                }
                else if (random == 2)
                {
                    item.Destroy();
                    Pickup.CreateAndSpawn(ItemType.KeycardGuard,item.Position);
                }
                else if (random == 3)
                {
                    item.Destroy();
                    CustomItem.TrySpawn(1101,item.Position,out var pickup);
                }
            }
            else if (item.Type == ItemType.KeycardGuard)
            {
                item.Destroy();
                CustomItem.TrySpawn(1100,item.Position,out var pickup);
            }
            else if (CustomItem.TryGet(item, out var customItem))
            {
                if (customItem.Id == 1100)
                {
                    item.Destroy();
                    Pickup.CreateAndSpawn(ItemType.KeycardMTFPrivate,item.Position);
                }
                else if (customItem.Id == 1101)
                {
                    item.Destroy();
                    item.Destroy();
                    Pickup.CreateAndSpawn(ItemType.KeycardChaosInsurgency,item.Position);
                }
            }
            else if (item.Type == ItemType.KeycardMTFPrivate)
            {
                item.Destroy();
                Pickup.CreateAndSpawn(ItemType.KeycardMTFOperative,item.Position);
            }
            else if (item.Type == ItemType.KeycardMTFOperative)
            {
                item.Destroy();
                Pickup.CreateAndSpawn(ItemType.KeycardMTFCaptain,item.Position);
            }
            else if (item.Type == ItemType.KeycardMTFCaptain)
            {
                item.Destroy();
                Pickup.CreateAndSpawn(ItemType.KeycardO5,item.Position);
            }
            else if (item.Type == ItemType.KeycardScientist)
            {
                item.Destroy();
                Pickup.CreateAndSpawn(ItemType.KeycardResearchCoordinator,item.Position);
            }
            else if (item.Type == ItemType.KeycardResearchCoordinator)
            {
                item.Destroy();
                Pickup.CreateAndSpawn(ItemType.KeycardContainmentEngineer,item.Position);
            }
            else if (item.Type == ItemType.KeycardContainmentEngineer)
            {
                item.Destroy();
                Pickup.CreateAndSpawn(ItemType.KeycardFacilityManager,item.Position);
            }
            else if (item.Type == ItemType.KeycardZoneManager)
            {
                item.Destroy();
