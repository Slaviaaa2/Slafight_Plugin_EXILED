...
public class EventHandler
{
    ...
    public void OnRoundStarted()
    {
        ...
        // Sergeyイベント実行中はAutoSCPスキップ（キック原因解消）
        ...
        {
            // 安全なPlayer初期化（null安全）
            foreach (var player in Player.List.ToList().Where(p => p?.IsConnected == true))
            {
                player.ShowHint("");
        ...
        Timing.CallDelayed(1f, () =>
        {
            ...
            // AutoSCP（Sergeyイベント中スキップ + 安全版）
            Timing.CallDelayed(3f, () =>
            {
                if (Plugin.Singleton.SpecialEventsHandler.NowEvent == SpecialEventType.SergeyMakarovReturns)
                    return;

                if (!Plugin.Singleton.EventHandler.IsScpAutoSpawnLocked)
                {
                    int scpCount = Player.List.Count(p => p?.IsConnected == true && p.Role?.Team == Team.SCPs);
                    Log.Debug($"[AutoSCP] SCP count = {scpCount}, players = {Player.List.Count(p => p?.IsConnected == true)}");

                    foreach (var p in Player.List.Where(p => p?.IsConnected == true))
                        Log.Debug($"[AutoSCP] {p.Nickname}: {p.Role?.Type} / {p.Role?.Team}");

                    if (scpCount == 0 && Player.List.Count(p => p?.IsConnected == true) > 0)
                    {
                        var alivePlayer = Player.List.FirstOrDefault(p => p?.IsAlive == true && p.IsConnected == true);
                        if (alivePlayer != null)
                        {
