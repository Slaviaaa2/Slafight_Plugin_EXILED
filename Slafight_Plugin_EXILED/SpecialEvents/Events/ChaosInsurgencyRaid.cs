...
public class ChaosInsurgencyRaid
{
    ...
    public void CIREvent()
    {
        ...
        Cassie.MessageTranslated("pitch_1.02 Danger Detected Unknown Forces in Gate A . Please Check pitch_.2 .g4 .g1 .g2","警告、不明な部隊がGate Aで検出されました。確認を",true);
        Timing.CallDelayed(8f, () =>
        {
            ...
            {
                if (eventPID != Slafight_Plugin_EXILED.Plugin.Singleton.SpecialEventsHandler.EventPID) return;
                Cassie.MessageTranslated("pitch_.8 First Order of Delta Command . Turn off all facilitys . Accepted .","<b><color=#228b22>DELTA COMMAND</color></b>の最初の指令：全施設の消灯 ...承認",false,false);
                foreach (Room room in Room.List)
                {
                    room.TurnOffLights();
                }
                Cassie.MessageTranslated("pitch_.8 Next Order . Turn off Tesla Gates . Accepted .","次の指令：テスラゲートの無効化 ...承認",false,false);
                teslaDisabled = true;
                Cassie.MessageTranslated("pitch_.8 All Agents . Work Time .","エージェント達よ、働く時間だ。",false,false);
