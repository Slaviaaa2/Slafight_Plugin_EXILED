...
public class OmegaWarhead
{
    public void OmegaWarheadEvent()
    {
        ...
        {
            if (eventPID != Slafight_Plugin_EXILED.Plugin.Singleton.SpecialEventsHandler.EventPID) return;
            Cassie.MessageTranslated("O5 Command has decided to halt containment breaches using alpha warheads. Continue evacuation.","O5評議会が<color=red>ALPHA WARHEAD</color>を用いた収容違反の一時解決を決定しました。起動までに引き続き非難をしてください。");
